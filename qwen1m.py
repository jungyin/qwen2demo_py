...
# model_name = "D:\\code\\transformer_models\\models--Qwen--Qwen2.5-7B-Instruct-1M"
...
def build_cache_random(model):
    ...
    num_kvheads = model.base_model.config.num_key_value_heads
    kvszie = 2 
    # 1,2,?,head_dim
...
position_ids = attention_mask.long().cumsum(-1) - 1
position_ids.masked_fill_(attention_mask == 0, 1)
past_key_values =torch.from_numpy( build_cache_random(model)).to(position_ids.device).to(torch.float32)
